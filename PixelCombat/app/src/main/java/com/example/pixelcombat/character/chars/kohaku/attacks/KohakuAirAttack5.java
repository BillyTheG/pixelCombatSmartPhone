...
public class KohakuAirAttack5 extends Attack {


    public KohakuAirAttack5(GameCharacter character, int id) {
        super(character, id);
    }
    ...
    public void process() {
        try {
            switch (character.getViewManager().getFrameIndex()) {
                case 0:
                    if (isSwitcher()) {
                        character.notifyObservers(new GameMessage(MessageType.SOUND, "kohaku_attack5", null, true));
                        setSwitcher(false);
                    }
                    break;
                case 2:
    ...
    public boolean isAttacking() {
        return character.getAttackManager().isAirAttacking5();
    }
