...
public class KohakuAirAttack4 extends Attack {


    public KohakuAirAttack4(GameCharacter character, int id) {
        super(character, id);
    }
    ...
    public void process() {
        try {
            switch (character.getViewManager().getFrameIndex()) {
                case 0:
                    if (isSwitcher()) {
                        character.notifyObservers(new GameMessage(MessageType.SOUND, "kohaku_attack4", null, true));
                        setSwitcher(false);
                    }
                    break;
                case 3:
    ...
    public boolean isAttacking() {
        return character.getAttackManager().isAirAttacking4();
    }
